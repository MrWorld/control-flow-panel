...
const Results = ({ resultData, pagination, actions, activeTab, loading, orderBy, order, onRequestSort, hideTabs }) => {
  ...
  const handleRedirectUserToProfile = id => {
    if (id === user.id) navigate(ROUTE_CONSTANTS.DASHBOARD.USERS.PROFILE.ROOT.ABSOLUTE)
    else navigate(ROUTE_CONSTANTS.DASHBOARD.MASTERS.USERS.GET_BY_DATA(id).ABSOLUTE)
  }
  ...
  return (
    <>
      <Box dir={'rtl'}
        ...
        pb={3}
      >
        {!roleGroupLoading && roleGroups.length > 0 && hideTabs && <TabsWrapper
        ...
        >
          {roleGroups.map((tab) => (
            <Tab key={tab.id} value={tab.id + ''} label={tab.name.trim().replace('_', ' ').toLowerCase()} />
          ))}
        </TabsWrapper>
                        ...
                        <TableCell>
                          <Box dir={'rtl'} display="flex" alignItems="center">
                            <Avatar
                              ...
                              src={imageURLCombiner(user.medias?.url) || staticImages.sampleUser}
                            />
                            <Box dir={'rtl'}>
                        ...
                        <TableCell align="right">
                          <Typography noWrap>
                            <Tooltip title={'View'} arrow>
                              <IconButton
                                disabled={!checkPermission('ADMIN:READ')}
                                onClick={() => checkPermission('ADMIN:READ') && handleRedirectUserToProfile(user.id)}
                                color="primary"
                              >
