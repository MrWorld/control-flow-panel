...
import { CancelOutlined, CheckOutlined, CropSharp } from "@mui/icons-material";
...
const Results = ({ resultData, pagination, actions, orderBy, order, onRequestSort }) => {
  ...
  const deleteRow = async (id) => {
    try {
      const response = await adminService.deleteParkingLocation(id)
                ...
                {resultData.map((data) => {
                  return (
                    <TableRow hover key={data.id}>
                      ...
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700">{data.keyTag}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700">{data.parking_block}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700">{data.customer}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Box dir={'rtl'}>
                      ...
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700">{data.amount}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700" color={data.status?.color}>
                            {data.status?.title}
                          </Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                            {data.PaymentStatus ? <CheckOutlined style={{color: 'green'}}/>: <CancelOutlined style={{color: 'red'}} />  }
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700" color={data.PaymentStatus ? 'green' : 'orange'}>
                          </Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell align="right">
                        <Typography noWrap>
                          <Tooltip title={"View"} arrow>
                            <IconButton
                              disabled={!checkPermission("BOOKING:READ")}
                              onClick={() =>
                                checkPermission("BOOKING:READ") &&
                                navigate(
                                  ROUTE_CONSTANTS.DASHBOARD.MASTERS.BOOKING.GET_BY_DATA(data.id).ABSOLUTE,
                                  { state: data }
                                )
                              }
