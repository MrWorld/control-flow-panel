...
const Results = ({ resultData, pagination, actions, refresh, orderBy, order, onRequestSort }) => {
  ...
  const deleteRow = async (id) => {
    try {
      const response = await adminService.deleteCarManufacturer(id)
      ...
      {resultData.length === 0 ? (
        <>
          <Typography
            ...
            align="center"
          >
            {"We couldn't find any car manufacturers matching your search criteria"}
          </Typography>
        </>
      ) : (
        <>
                      ...
                      <TableCell align="right">
                        <Typography noWrap>
                          <Tooltip title={"View"} arrow>
                            <IconButton
                              disabled={!checkPermission("CAR_MANUFACTURER:UPDATE")}
                              onClick={() =>
                                checkPermission("CAR_MANUFACTURER:UPDATE") &&
                                navigate(
                                  ROUTE_CONSTANTS.DASHBOARD.MASTERS.CAR_MANUFACTURER.UPDATE.ROOT.ABSOLUTE,
                                  { state: data }
                                )
                              }
...
const ListActiveStatus = ({ data }) => {
  ...
  const handleToggle = async () => {
    ...
    try {
      setLoading(true);
      await adminService.toggleCarManufacturer(data.id, tempData);
      toast(
