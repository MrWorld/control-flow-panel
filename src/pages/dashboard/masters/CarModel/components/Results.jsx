...
const Results = ({ resultData, pagination, actions, orderBy, order, onRequestSort }) => {
  ...
  const deleteRow = async (id) => {
    try {
      const response = await adminService.deleteCarModel(id)
      ...
      {resultData.length === 0 ? (
        <>
          <Typography
            ...
            align="center"
          >
            {"We couldn't find any car model matching your search criteria"}
          </Typography>
        </>
      ) : (
        <>
                ...
                {resultData.map((data) => {
                  return (
                    <TableRow hover key={data.id}>
                      ...
                      <TableCell>
                        <Box dir={'rtl'}>
                          <Typography fontWeight="700">{data.vehicleManufacturer?.name}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Box dir={'rtl'}>
                      ...
                      <TableCell align="right">
                        <Typography noWrap>
                          <Tooltip title={"View"} arrow>
                            <IconButton
                              disabled={!checkPermission("CAR_MODEL:UPDATE")}
                              onClick={() =>
                                checkPermission("CAR_MODEL:UPDATE") &&
                                navigate(
                                  ROUTE_CONSTANTS.DASHBOARD.MASTERS.CAR_MODEL.UPDATE.ROOT.ABSOLUTE,
                                  { state: data }
                                )
                              }
...
const ListActiveStatus = ({ data }) => {
  ...
  const handleToggle = async () => {
    ...
    try {
      setLoading(true);
      await adminService.toggleCarModel(data.id, tempData);
      toast(
