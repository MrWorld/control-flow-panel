...
const AddUpdatePage = ({ addNew, formData }) => {
  ...
  const onCreate = async (
    ...
    try {
      await adminService.addStation(formData);
      toast("با موفقیت ساخته شد", "success");
      setSubmitting(false);
      resetForm();
      setStatus({ success: true });
      navigate(ROUTE_CONSTANTS.DASHBOARD.STATIONS.ROOT.ABSOLUTE, {
        replace: true,
      });
    } catch (err) {
  ...
  const onUpdate = async (
    ...
    try {
      await adminService.updateStation(id, formData);
  ...
  const getDetail = async () => {
    try {
      console.log("here");
      setLoading(true);
      const response = await adminService.getStationDetail(id);
      ...
      >
        <Grid item>
          <Typography variant="h3" component="h3" gutterBottom>
            {addNew ? "ایجاد دستگاه جدید" : "ویرایش دستگاه"}
          </Typography>
        </Grid>
      </Grid>
      <Grid
            ...
            <Card style={{ padding: "25px" }}>
              <Formik
                ...
                validationSchema={Yup.object().shape({
                  name: Yup.string().max(255).required("نام دستگاه اجباری است"),
